...
def getGraph():
    ...
    with g.as_default():
        ...
        # tf Graph input
        tf_x = tf.placeholder(tf.float32, [None, number_of_features])
        
        
        tf_y1 = tf.placeholder(tf.float32, [None, NUMBER_OF_CLASSES])
        
        #leaving 1 for blank
        tf_y2 = tf.placeholder(tf.float32, [None, NUMBER_OF_CLASSES+1])
        tf_y3 = tf.placeholder(tf.float32, [None, NUMBER_OF_CLASSES+1])
        
        tf_length = tf.placeholder('uint8')
        ...
        last_conv_layer =  convolution_layers(tf_x, weights, biases, keep_prob, INPUT_DIMENSION)
        last_fc_layer_d1 = classifier_FC_d1(last_conv_layer, weights, biases, keep_prob)
        last_fc_layer_d2 = classifier_FC_d2(last_conv_layer, weights, biases, keep_prob)
        last_fc_layer_d3 = classifier_FC_d3(last_conv_layer, weights, biases, keep_prob)
        ...
        digit1_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(digit1_logits, tf_y1))
        digit2_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(digit2_logits, tf_y2))
        digit3_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(digit3_logits, tf_y3))
        
        overall_loss = digit1_loss + digit2_loss + digit3_loss
        
        optimiser = op_algorithm(starting_learning_rate).minimize( overall_loss) 
        
        
        correct_digit1_prediction = tf.equal(tf.argmax(digit1_logits,1) , tf.argmax(tf_y1,1))
        digit1_accuracy = tf.reduce_mean(tf.cast(correct_digit1_prediction, "float"))
        
        correct_digit2_prediction = tf.equal(tf.argmax(digit2_logits,1) , tf.argmax(tf_y2,1))
        digit2_accuracy = tf.reduce_mean(tf.cast(correct_digit2_prediction, "float"))
        
        correct_digit3_prediction = tf.equal(tf.argmax(digit3_logits,1) , tf.argmax(tf_y3,1))
        digit3_accuracy = tf.reduce_mean(tf.cast(correct_digit3_prediction, "float"))

    return g

def init_interactive_session(graph):
    return tf.InteractiveSession(graph=graph)

def load_saved_model(path_to_saved_model, session):
    saver = tf.train.Saver()
    ckpt = tf.train.get_checkpoint_state(path_to_saved_model)
    saver.restore(session, ckpt.model_checkpoint_path)

def run_training_epoch(session, x, y1, y2, y3, keep_prob):
    l = None
    feed_dict = { tf_length: l, tf_x: x, tf_y1: y1, tf_y2: y2, tf_y3: y3, keep_prob:keep_p}
        
    session.run(optimiser,feed_dict=feed_dict)

def validate_model(session, x, y1, y2, y3):
    d1, d2, d3 = session.run([digit1_accuracy, digit2_accuracy, digit3_accuracy], \
                     feed_dict={tf_x: x, tf_y1: y1, y2: y2, tf_y3: y3, keep_prob:1.0})
    print "Validation Step: d1 acc=%0.3f d2 acc=%0.3f d3 acc=%0.3f" % (d1, d2, d3 )
